...
use one_wire_bus::{OneWire, OneWireError, OneWireResult, Triplet};
...
impl<I2C, E> DS2482<I2C, E>
    where I2C: Read<Error=E> + Write<Error=E> + WriteRead<Error=E>
{
    pub fn new(i2c: I2C, addr: u8) -> DS2482<I2C, E> {
        DS2482 { i2c, addr }
    }
    ...
    pub fn ds2482_device_reset(&mut self) -> DS2482Result<(), E> {
        ...
        if (status & STATUS_RST) == 0 {
            return Err(DS2482Error::DeviceResetFailed);
        }
    ...
    pub fn ds2482_write_config(&mut self, config: u8) -> DS2482Result<(), E> {
        ...
        if config != read_config {
            ...
            return Err(DS2482Error::WriteConfigFailed);
        }
...
impl<I2C, E> OneWire for DS2482<I2C, E>
    where I2C: Read<Error=E> + Write<Error=E> + WriteRead<Error=E>
{
    fn reset(&mut self, delay: &mut dyn DelayUs<u16>) -> OneWireResult<bool> {
    ...
    fn read_bit(&mut self, delay: &mut dyn DelayUs<u16>) -> OneWireResult<bool> {
    ...
    fn read_byte(&mut self, delay: &mut dyn DelayUs<u16>) -> OneWireResult<u8> {
        self.ds2482_wait_on_busy(delay)?;
    ...
    fn write_bit(&mut self, bit: bool, delay: &mut dyn DelayUs<u16>) -> OneWireResult<()> {
    ...
    fn write_byte(&mut self, value: u8, delay: &mut dyn DelayUs<u16>) -> OneWireResult<()> {
        self.ds2482_wait_on_busy(delay)?;
        Ok(self.ds2482_write_bytes(&[COMMAND_1WWB, value])?)
    }

    fn triplet(&mut self, dir_bit: bool, delay: &mut dyn DelayUs<u16>) -> OneWireResult<Triplet> {
